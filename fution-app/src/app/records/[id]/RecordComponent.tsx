...
function RecordComponent({ record, debt }: { record?: RecordDetailType, debt?: {
    init: string;
    curr: string;
    next: string[];
} }) {
return (<>
    ...
    <div className="text-sm">
        ...
        { record?.status === "PENDING" && <p className="mb-2">This is an ongoing negotiation process. Here are the current agreed values.</p> }
        { record?.status === "PENDING" && (
            <div className="flex justify-between items-center pr-3 font-semibold text-base">
                <p>Amount</p> 
                <p>{ record.amount ? toDollarFormat(Number(record.amount.toString())) : ""}</p> 
            </div>
        )}
        { record?.status === "PENDING" && (
            <div className="flex justify-between items-center pr-3 font-semibold text-base">
    ...
    <div className="ml-auto mr-0 mb-4 font-semibold text-sm">
        ...
        { record?.status === "PENDING" && (<>
            <Link href="/home" className="bg-red-200 rounded-lg hover:bg-red-300 flex-grow-0 py-2 px-5 ml-2 shadow">
                Reject
            </Link>
            <Link href="/home" className="bg-emerald-200 rounded-lg hover:bg-emerald-300 flex-grow-0 py-2 px-5 ml-2 shadow">
                Negotiate
        ...
        <Link href="/home" className="bg-sky-200 rounded-lg hover:bg-sky-300 flex-grow-0 py-2 px-5 ml-2 shadow">
            Back
        </Link>
    </div>
</>)
